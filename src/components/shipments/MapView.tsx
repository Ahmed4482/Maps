  ...
  useEffect(() => {
    ...
    if (trackingData) {
      ...
      if (trackingData.ports.origin) {
        ...
        const originInfoWindow = new window.google.maps.InfoWindow({
          content: `
            <div style="padding: 0 12px 12px 12px; margin-top: -8px; font-family: sans-serif; width: 300px;">
              <div style="font-weight: bold; color: #0A5C3A; margin-bottom: 8px; font-size: 14px; padding-top: 4px;">📍 Origin</div>
              <div id="origin-city-photo" style="margin-bottom: 8px; display: none;">
                <img id="origin-city-photo-img" style="width: 100%; height: 180px; object-fit: cover; border-radius: 4px;" alt="City photo" />
              </div>
              <div style="font-size: 12px; color: #333; margin-bottom: 6px; font-weight: 600;">${trackingData.ports.origin.name}</div>
              <div style="font-size: 11px; color: #666; margin-bottom: 4px;">
                <strong>Port Code:</strong> ${trackingData.ports.origin.code || 'N/A'}<br/>
                <strong>Coordinates:</strong> ${trackingData.ports.origin.lat.toFixed(4)}, ${trackingData.ports.origin.lng.toFixed(4)}
              </div>
            </div>
          `,
        });
      ...
      if (trackingData.ports.destination) {
        ...
        const destInfoWindow = new window.google.maps.InfoWindow({
          content: `
            <div style="padding: 0 12px 12px 12px; margin-top: -8px; font-family: sans-serif; width: 300px;">
              <div style="font-weight: bold; color: #0A5C3A; margin-bottom: 8px; font-size: 14px; padding-top: 4px;">🎯 Destination</div>
              <div id="destination-city-photo" style="margin-bottom: 8px; display: none;">
                <img id="destination-city-photo-img" style="width: 100%; height: 180px; object-fit: cover; border-radius: 4px;" alt="City photo" />
              </div>
              <div style="font-size: 12px; color: #333; margin-bottom: 6px; font-weight: 600;">${trackingData.ports.destination.name}</div>
              <div style="font-size: 11px; color: #666; margin-bottom: 4px;">
                <strong>Port Code:</strong> ${trackingData.ports.destination.code || 'N/A'}<br/>
                <strong>Coordinates:</strong> ${trackingData.ports.destination.lat.toFixed(4)}, ${trackingData.ports.destination.lng.toFixed(4)}
              </div>
            </div>
          `,
        });
      ...
      trackingData.ports.transshipment.forEach((port, portIndex) => {
        ...
        const transshipmentInfoWindow = new window.google.maps.InfoWindow({
          content: `
            <div style="padding: 0 12px 12px 12px; margin-top: -8px; font-family: sans-serif; width: 300px;">
              <div style="font-weight: bold; color: #f59e0b; margin-bottom: 8px; font-size: 14px; padding-top: 4px;">🔄 Transshipment</div>
              <div id="transshipment-city-photo-${portIndex}" style="margin-bottom: 8px; display: none;">
                <img id="transshipment-city-photo-img-${portIndex}" style="width: 100%; height: 180px; object-fit: cover; border-radius: 4px;" alt="City photo" />
              </div>
              <div style="font-size: 12px; color: #333; margin-bottom: 6px; font-weight: 600;">${port.name}</div>
              <div style="font-size: 11px; color: #666; margin-bottom: 4px; line-height: 1.5;">
                <strong>Port Code:</strong> ${port.code || 'N/A'}<br/>
                <strong>Location:</strong> ${port.city || ''}${port.city && port.country ? ', ' : ''}${port.country || ''}<br/>
                <strong>Coordinates:</strong> ${port.lat.toFixed(4)}, ${port.lng.toFixed(4)}<br/>
                <strong>Arrival:</strong> ${new Date(port.arrivalDate).toLocaleDateString()}<br/>
                <strong>Departure:</strong> ${new Date(port.departureDate).toLocaleDateString()}<br/>
                ${port.vesselName ? `<strong>Vessel:</strong> ${port.vesselName}<br/>` : ''}
                ${port.vesselImo ? `<strong>IMO:</strong> ${port.vesselImo}` : ''}
              </div>
            </div>
          `,
        });
        ...
        img.onload = () => {
          ...
          if (ctx) {
            ...
            const vesselInfoWindow = new window.google.maps.InfoWindow({
              content: `
                <div style="padding: 0 12px 12px 12px; margin-top: -8px; font-family: sans-serif; width: 300px;">
                  <div style="font-weight: bold; color: #3b82f6; margin-bottom: 8px; font-size: 14px; padding-top: 4px;">🚢 Current Position</div>
                  ...
                  <div style="font-size: 11px; color: #666; margin-bottom: 4px; line-height: 1.5;">
                    ...
                    <strong>Coordinates:</strong> ${trackingData.currentPosition.lat.toFixed(4)}, ${trackingData.currentPosition.lng.toFixed(4)}<br/>
    ...
    points.forEach((point, idx) => {
      ...
      const infoWindow = new window.google.maps.InfoWindow({
        content: `
          <div style="padding: 0 12px 12px 12px; margin-top: -8px; font-family: sans-serif; width: 300px;">
            <div style="font-weight: bold; color: #0A5C3A; margin-bottom: 8px; font-size: 14px; padding-top: 4px;">${title}</div>
            ...
            ${isOrigin ? `<div style="font-size: 12px; color: #666; margin-bottom: 4px;">${activeShipment.origin}</div>` : ''}
            ${isDestination ? `<div style="font-size: 12px; color: #666; margin-bottom: 4px;">${activeShipment.destination}</div>` : ''}
            ${isOriginCoastal ? `<div style="font-size: 12px; color: #666; margin-bottom: 4px;">${extractCountry(activeShipment.origin)} Port</div>` : ''}
            ${isSeaWaypoint ? `<div style="font-size: 12px; color: #666; margin-bottom: 4px;">Ocean Route</div>` : ''}
            ${isDestCoastal ? `<div style="font-size: 12px; color: #666;">${extractCountry(activeShipment.destination)} Port</div>` : ''}
          </div>
        `,
      });
